...
// RotateKeypair rotates the endpoint node's WireGuard keypair atomically.
//
// Flow (local tracker #125 fix — endpoint half of 4-party coordinated rotation):
//  1. Validate new_private_key length == 32.
//  2. Verify endpoint mode: stateProvider must implement NodeStatePersister AND
//     paramApplier must be set. Master/client modes return Unimplemented.
//  3. Load current NodeState from disk to capture oldPriv + oldPub for rollback.
//  4. Persist new keypair to node.yml atomically via PersistKeypair.
//  5. Rebind live WireGuard interface PrivateKey via ApplyParams (UAPI).
//  6. On UAPI failure: restore old keypair via PersistKeypair (best-effort),
//     then best-effort restore via ApplyParams(oldPriv); return Internal error.
//
// Private-key material is NEVER logged at any zerolog level (NFR-1). Pubkey
// prefixes (first 8 hex chars) are logged at Info level on success.
func (h *AgentHandler) RotateKeypair(_ context.Context, req *proto.RotateKeypairRequest) (*proto.RotateKeypairResponse, error) {
	if req == nil {
		return nil, status.Error(codes.InvalidArgument, "request is required")
	}

	rawKey := req.GetNewPrivateKey()
	if len(rawKey) != 32 {
		return nil, status.Error(codes.InvalidArgument, "new_private_key must be exactly 32 bytes")
	}

	// Endpoint-mode gating: only EndpointRunner implements NodeStatePersister.
	// Master and client modes do not, so the type assertion fails → Unimplemented.
	persister, ok := h.stateProvider.(NodeStatePersister)
	if !ok || h.paramApplier == nil {
	...
	if loadErr != nil {
		return nil, status.Errorf(codes.Internal, "load current keypair: %v", loadErr)
	}

	// Construct new key from request bytes.
	newPriv, keyErr := wg.NewKey(rawKey)
	if keyErr != nil {
		return nil, status.Errorf(codes.InvalidArgument, "parse new_private_key: %v", keyErr)
	}
	newPub := newPriv.PublicKey()

	// Persist new keypair atomically to node.yml (.tmp + rename, mode 0600).
	if err := persister.PersistKeypair(newPriv, newPub); err != nil {
		h.logger.Error().Err(err).Msg("rotate keypair: persist node state failed")
		return nil, status.Errorf(codes.Internal, "persist keypair: %v", err)
	}

	// Rebind the live endpoint interface to the new private key via UAPI.
	// tunnelName defaults to "wg0" (the canonical endpoint interface name).
	tunnelName := strings.TrimSpace(req.GetTunnelName())
	if tunnelName == "" {
		tunnelName = "wg0"
	}
	if applyErr := h.paramApplier.ApplyParams(tunnelName, wg.Config{PrivateKey: &newPriv}); applyErr != nil {
		h.logger.Error().Err(applyErr).
			Str("new_pub_prefix", hex.EncodeToString(newPub[:])[:8]).
			Msg("rotate keypair: UAPI apply failed; rolling back disk state")

		// Rollback step 1: restore old keypair on disk.
		diskErr := persister.PersistKeypair(oldPriv, oldPub)

		// Rollback step 2: best-effort re-apply old private key via UAPI.
		// Ignore error — runtime may already be inconsistent; disk is authoritative.
		_ = h.paramApplier.ApplyParams(tunnelName, wg.Config{PrivateKey: &oldPriv})

		if diskErr != nil {
			return nil, status.Errorf(codes.Internal,
				"apply keypair: %v; disk rollback also failed: %v", applyErr, diskErr)
		}
		return nil, status.Errorf(codes.Internal, "apply keypair: %v; rolled back", applyErr)
	}

	h.logger.Info().
		Str("old_pub_prefix", hex.EncodeToString(oldPub[:])[:8]).
		Str("new_pub_prefix", hex.EncodeToString(newPub[:])[:8]).
		Str("tunnel", tunnelName).
		Msg("endpoint keypair rotated")

	newPubBytes := make([]byte, 32)
	copy(newPubBytes, newPub[:])

	return &proto.RotateKeypairResponse{
		Success:      true,
