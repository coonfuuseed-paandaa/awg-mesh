...
- F-005 (v1.16+): two-client dpext fixture extension closes the FR-1 + FR-6 fixture-N/A gap from F-004 CR-003. `tests/simulation/data-plane-extended.sh` brings up `dpext-client-01` + `dpext-client-02` (`awg-mesh-node --mode client`) on the same dpext compose project; FR-1 module drives 200 UDP flows split 50/50 across both clients (cross-source ECMP regression catch — US4); FR-6 module exercises healthcheck-driven nexthop migration from a real client. F-005 NOT in release gate yet (per CR-003 conditionality + TD-2026-04-30-F-004-INTEGRATION-FINDINGS deferred bundle).
