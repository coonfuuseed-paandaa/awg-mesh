...
### Added
- **Go module v2 import path** - the module path is now `github.com/coonfuuseed-paandaa/awg-mesh/v2`, matching the `v2.0.0` tag line and preserving `go install ...@v2.0.0` compatibility for Go modules.
- **Release-facing documentation refresh** - `README.md`, localized README variants, `mesh-topology.example.yml`, and simulation docs now describe the v2 role-agnostic `mesh-ctl node ...` workflow and current release gates instead of removed v1 role commands.
