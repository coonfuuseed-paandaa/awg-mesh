...
# Short names — `CTR_CLIENT_*` adds the COMPOSE_PROJECT prefix below to avoid
# the dpext-dpext-client-01 double-prefix bug.
CLIENT_01="client-01"
CLIENT_02="client-02"
...
topo::prepare_nodes() {
    ...
    for node in "${MASTER_RU_01}" "${MASTER_RU_02}"; do
        logfile="/tmp/dpext-master-${node}-prepare.log"
        ${MESHCTL_BIN} \
            --topology "${TOPO_FILE}" \
            --config-dir "${CTL_CONFIG_DIR}" \
            master prepare "${node}" > "${logfile}" 2>&1 || {
            printf '[topo] ERROR: mesh-ctl master prepare %s failed (log %s):\n' "${node}" "${logfile}" >&2
            tail -n 20 "${logfile}" >&2 || true
            return 3
        }
    done
    for node in "${ENDPOINT_US_01}" "${ENDPOINT_ASIA_01}" "${ENDPOINT_ASIA_02}"; do
        logfile="/tmp/dpext-endpoint-${node}-prepare.log"
        ${MESHCTL_BIN} \
            --topology "${TOPO_FILE}" \
            --config-dir "${CTL_CONFIG_DIR}" \
            endpoint prepare "${node}" > "${logfile}" 2>&1 || {
            printf '[topo] ERROR: mesh-ctl endpoint prepare %s failed (log %s):\n' "${node}" "${logfile}" >&2
            tail -n 20 "${logfile}" >&2 || true
            return 3
        }
    done
    ...
    for node in "${CLIENT_01}" "${CLIENT_02}"; do
        logfile="/tmp/dpext-client-${node}-prepare.log"
        ( ${MESHCTL_BIN} \
            --topology "${TOPO_FILE}" \
            --config-dir "${CTL_CONFIG_DIR}" \
            client prepare "${node}" > "${logfile}" 2>&1 \
            || { printf '[topo] ERROR: mesh-ctl client prepare %s failed (log %s):\n' "${node}" "${logfile}" >&2; tail -n 20 "${logfile}" >&2; exit 3; }
        ) &
        client_prepare_pids+=("$!")
    done
    local pid
...
topo::generate_compose() {
    # F-005 NFR-6 opt-in: DPEXT_INTERNAL_NETWORK=1 forces internal: true on the
    # dpext network (no public-internet egress). Default OFF because cold-image
    # masters may need apk add tcpdump etc; flipping to internal: true breaks
    # that. NFR-6 full enforcement deferred to a follow-up CR after image
    # pre-bundles all tooling.
    local DPEXT_INTERNAL_LINE=""
    if [[ "${DPEXT_INTERNAL_NETWORK:-0}" == "1" ]]; then
        DPEXT_INTERNAL_LINE="    internal: true"
    fi
...
topo::init_nodes() {
    ...
    for n in "${MASTER_RU_01}" "${MASTER_RU_02}"; do
        logfile="/tmp/dpext-master-${n}-init.log"
        ${MESHCTL_BIN} \
            --topology "${TOPO_FILE}" \
            --config-dir "${CTL_CONFIG_DIR}" \
            master init "${n}" > "${logfile}" 2>&1 || {
            printf '[topo] ERROR: master init %s failed (log %s):\n' "${n}" "${logfile}" >&2
            tail -n 20 "${logfile}" >&2 || true
            return 2
        }
    done
    ...
    for n in "${ENDPOINT_US_01}" "${ENDPOINT_ASIA_01}" "${ENDPOINT_ASIA_02}"; do
        logfile="/tmp/dpext-endpoint-${n}-init.log"
        ${MESHCTL_BIN} \
            --topology "${TOPO_FILE}" \
            --config-dir "${CTL_CONFIG_DIR}" \
            endpoint init "${n}" > "${logfile}" 2>&1 || {
            printf '[topo] ERROR: endpoint init %s failed (log %s):\n' "${n}" "${logfile}" >&2
            tail -n 20 "${logfile}" >&2 || true
            return 2
        }
    done
