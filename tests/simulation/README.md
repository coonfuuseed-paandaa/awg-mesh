...
| `tests/critical/f011-peer-identity-handshake.sh` | F-011 peer identity regression: control-plane snapshot overrides stale clientd cache, endpoint-aware peer config reaches both sides, master-first packet transit works, and the peer reports a non-zero handshake timestamp. |
